...
| `convnum` | the whole [convnum](/api/readme/) library: `convnum.toRoman(cn)`, `convnum.toEnglishWords(n)`, `convnum.toChineseWords(cn)`, `convnum.toHex(n)`, `convnum.toMonth(cn)`, `convnum.fromRoman(s)`, `convnum.anyToNumber(s)`, … |
